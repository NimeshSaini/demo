...
@HiltViewModel
class FeedDetailViewModel @Inject constructor(var repository: Repository, var utkashRoom: UtkashRoom) :
    ViewModel() {
    ...
    var bodydata: MutableLiveData<String> = MutableLiveData()
     ...
     val _relatedCourse = MutableLiveData<List<NewCourseData>>()
    val relatedCourse: LiveData<List<NewCourseData>> = _relatedCourse
    ...
    fun getPostData() {
        viewModelScope.launch(handler) {
            val result = repository.getPostdata(bodydata.value.toString())
            try {
